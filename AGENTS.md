...
1. `README.md`
2. 涉及前端 UI 时阅读 `DESIGN.md`
3. 与任务直接相关的 `docs/` 方案
4. `progress/tasks/*.json` 中对应任务
5. `progress/risks.json` 中关联风险
...
前端保持现有 React/Vite 单页应用结构。所有生产 UI 必须遵循根目录 `DESIGN.md`；界面文案、按钮、状态和错误默认使用中文，专有名词、协议字段和代码标识可保留英文。
...
- `DESIGN.md` 是前端视觉、交互、文案、响应式、动效和无障碍的强制规范；任何生产 UI 新增或修改都必须逐项遵循。
- 修改 UI 前先识别并复用设计 tokens 与共享组件。新增或修改区域不得硬编码颜色、圆角、阴影、间距层级或动效时长。
- 图标统一使用 `lucide-react`；禁止 Emoji、手绘 SVG、图标字体或混用多套图标库。图标按钮必须有 `aria-label`，陌生图标必须有 Tooltip。
- 保持桌面工作台的信息密度，移动端使用逐层导航和 Sheet 完整承载功能；不得把桌面多栏简单纵向堆叠。
- 删除营销口号、英文眉题、重复说明和填充内容。状态、错误、风险与恢复动作必须短、具体，并出现在相关操作附近。
- 交互刷新默认保留旧内容、选择、滚动位置和稳定布局；禁止整页闪白、无理由清空或重新挂载造成跳动。
- 动画以稳定 60fps 和状态连续为目标，主要只改变 `transform` 与 `opacity`，并完整支持 `prefers-reduced-motion`。
